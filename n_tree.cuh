...
struct n_tree_cuda{
    ...
    n_tree_cuda(torch::Tensor &e, torch::Tensor &d, torch::Tensor &xm,std::string &cuda_str): data(d),device(cuda_str){
        xmin = xm.to(device);
        edge = e.to(device);
        dim = data.size(1);
        dim_fac = pow(2,dim);
        ...
        largest_box_n = (int) unique_counts.max().item();
        avg_nr_points = (float) unique_counts.mean().item();
