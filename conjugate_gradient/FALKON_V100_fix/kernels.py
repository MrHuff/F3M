...
import torch
from conjugate_gradient.FALKON_V100_fix.keops_mixin_custom import KeopsKernelMixin
...
class GaussianKernelV100(L2DistanceKernel, KeopsKernelMixin):
    ...
    def __repr__(self):
        return f"GaussianKernelV100(sigma={self.sigma})"

    def __str__(self):
        return f"Gaussian kernel<{self.sigma}>"


class LaplacianKernelV100(GaussianKernelV100):
    ...
    def __init__(self, sigma: float, opt: Optional[BaseOptions] = None):
        ...
        if self.gaussian_type != 'single':
            raise NotImplementedError("LaplacianKernelV100 is only implemented for scalar length-scale.")
    ...
    def __repr__(self):
        return f"LaplacianKernelV100(sigma={self.sigma})"

    def __str__(self):
        return f"Laplaciankernel<{self.sigma}>"


class MaternKernelV100(GaussianKernelV100):
    ...
    Notes
    -----
    While for `nu = float('inf')` this kernel is equivalent to the :class:`GaussianKernelV100`,
    the implementation is more general and using the :class:`GaussianKernelV100` directly
    ...
    def __init__(self,
        ...
        if isinstance(nu, torch.Tensor):
            nu = float(nu.item())
        if nu not in MaternKernelV100._valid_nu_values:
            raise ValueError(f"The given value of nu = {nu} can only take "
                             f"values {MaternKernelV100._valid_nu_values}.")
        self.nu = nu

        self.sigma, self.gaussian_type = self._get_sigma_kt(sigma)
        if self.gaussian_type not in {'single', 'diag'}:
            raise ValueError(f"'{self.gaussian_type}' covariance matrix not allowed for the MaternKernelV100 class.")

        # Cannot use the distk variants since they are not ready to deal with extra-mem requirements.
        # This could mean that the MaternKernelV100 implementation with nu=inf is slightly less efficient
        # than the GaussianKernelV100.
        self.kernel_type = "l2-multi-distance"
    ...
    def __repr__(self):
        return f"MaternKernelV100(sigma={self.sigma}, nu={self.nu:.1f})"
