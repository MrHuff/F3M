import numpy as np
from math import pi
from gpytorch.kernels import RBFKernel,Kernel,PolynomialKernel
import torch
...
def interpolation_poly_experiment(X,Y,b,params,nr_of_nodes):
    dot_prod_bound=params['bound']
    d = params['d']
    c = torch.tensor(params['c']).float()
    rbf = PolynomialKernel(power=d)
    rbf.offset(c)
    with torch.no_grad():
        res = rbf(X,Y)@b
        res = res.squeeze()
...
def run_poly_experiment(d,eff_far_field,nr_intpol,poly_d,poly_c,dot_bound):

    #TODO: think about how to bound the dot-product or the (-1+c+xy) term essentially
    ...
    rel_err,abs_error = interpolation_poly_experiment(X,Y,b,params,nr_intpol)
    return rel_err,abs_error,eff_far_field,nr_intpol**d
...
if __name__ == '__main__':
    ...
    for el in itertools.product(d_list, ls_list, node_list):
        ...
        data_list.append([el[0], eff_far_field, nodes, rel_err.item(), abs_error.item()])
    df = pd.DataFrame(data_list, columns=columns)
    df.to_csv("poly_experiment_error.csv")
