...
//    std::cout<<warmup_duration_2.count()<<std::endl;
    ...
    bool old_mode = (bool) std::stoi(argv[13]);


    if (job==1){
        benchmark_1<float,3>(n,min_points,threshold,a,b,ls,nr_of_interpolation_nodes,var_comp,eff_var,small_field_limit,old_mode,fname); //Can't do a billion points for 3 dim...
    }
    if(job==2){
        benchmark_2<float,5>(n,min_points,threshold,a,b,ls,nr_of_interpolation_nodes,var_comp,eff_var,small_field_limit,old_mode,fname);//Weird curse of dimensionality... Should scale linearly in diemnsions...
    }
    if(job==3){
        benchmark_3<float,5>(n,min_points,threshold,a,b,ls,nr_of_interpolation_nodes,var_comp,eff_var,small_field_limit,old_mode,fname);//Weird curse of dimensionality... Should scale linearly in diemnsions...
    }
    if(job==4){
        benchmark_4<float,3>(n,min_points,threshold,a,b,ls,nr_of_interpolation_nodes,var_comp,eff_var,small_field_limit,old_mode,fname);//Weird curse of dimensionality... Should scale linearly in diemnsions...
    }
    if(job==5){
