...
//    torch::Tensor b_train = read_csv<float>("Y_train.csv",11619,1); something wrong with data probably...
    torch::Tensor X_train = torch::rand({1000000,nd});//.to(device_cuda); //Something fishy going on here, probably the boxes stuff...
    torch::Tensor b_train = torch::randn({1000000,1});//to(device_cuda);
...
//    float tol = 1e-6;
    ...
    FFM_object<float> ffm_obj = FFM_object<float>(X_train, X_train, ls, lambda, device_cuda); //FMM object
//    FFM_object<float> ffm_obj_grad = FFM_object<float>(X,X,ls,op_grad,lambda,device_cuda);
    exact_MV<float> exact_ref = exact_MV<float>(X_train, X_train, ls,  lambda, device_cuda); //Exact method reference
