...
//    std::cout<<warmup_duration_2.count()<<std::endl;
    ...
    if (job==1){
        benchmark_1<float,3>(n,min_points,threshold,a,b,ls,nr_of_interpolation_nodes,var_comp,eff_var,small_field_limit,fname,seed); //Can't do a billion points for 3 dim...
    }
    if(job==2){
        benchmark_2<float,5>(n,min_points,threshold,a,b,ls,nr_of_interpolation_nodes,var_comp,eff_var,small_field_limit,fname,seed);//Weird curse of dimensionality... Should scale linearly in diemnsions...
    }
    if(job==3){
