...
def dataset_X(penalty_in):
    dirname = f'FALKON_dataset_test_{penalty_in}'
    counter = 0
    M=100000
    N=1000000000
    d=2
    if not os.path.exists(dirname):
        os.makedirs(dirname)
    for seed in [1]:
        for seed,eff_var_limit,nr_of_interpolation,penalty in zip([1,2,3],[2, 2, 2],[16,16,16],[penalty_in,penalty_in,penalty_in]):
...
def dataset_X_bench(penalty_in):

    # pykeops.clean_pykeops()
    # pykeops.test_torch_bindings()
    dirname = f'FALKON_dataset_benchmark_test_{penalty_in}'
    counter = 0
    N=1000000000
    M=100000
    d=2
    if not os.path.exists(dirname):
        os.makedirs(dirname)
    for seed,_,nr_of_interpolation,penalty in zip([1,2,3],[2, 2, 2],[16,16,16],[penalty_in,penalty_in,penalty_in]):
        problem_set = torch.load(f'real_problem_N={N}_seed={seed}.pt')
        X = problem_set['X']
        Y = problem_set['y']
        ls = problem_set['ls']
...
if __name__ == '__main__':
    ...
    if idx==0:
        dataset_X(penalty_in)
    elif idx==1:
        dataset_X_bench(penalty_in)
