...
__device__ uint32_t hash_2(uint32_t k,const int * input_kHashTableCapacity)
{
    k ^= k >> 16;
    k *= 0x85ebca6b;
    k ^= k >> 13;
...
KeyValue* create_hashtable_size(int input_kHashTableCapacity)
{
    // Allocate memory
    KeyValue* hashtable;
    cudaMalloc(&hashtable, sizeof(KeyValue) * input_kHashTableCapacity);

    // Initialize hash table to empty
    static_assert(kEmpty == 0xffffffff, "memset expected kEmpty=0xffffffff");
    cudaMemset(hashtable, 0xff, sizeof(KeyValue) * input_kHashTableCapacity);

    return hashtable;
}

__device__ void device_insert(KeyValue* hashtable,int key, int value,int * table_size){
    {
        uint32_t slot = hash_2(key,table_size);
...
__device__ int device_lookup(KeyValue* hashtable,int & key,int * table_size){
    uint32_t slot = hash_2(key,table_size);
    while (true)
    {
